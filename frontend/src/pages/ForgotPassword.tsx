...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setStatus('If an account exists, a reset link has been sent to the email address.');
    } catch (err: any) {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white dark:bg-slate-800 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-slate-200 dark:border-slate-700">
          <form className="space-y-6" onSubmit={handleSubmit}>
            {status && <div className="text-sm text-center p-2 bg-slate-100 dark:bg-slate-700 rounded text-slate-800 dark:text-slate-200">{status}</div>}
